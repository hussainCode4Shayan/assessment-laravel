...
class QuizRepository
{
    ...
    public function getQuizSessionStats($quizId, $scheduleId)
    {
        ...
        return DB::table('quiz_sessions')
            ...
            ->selectRaw("count(case when results>>'$.pass_or_fail'= 'Passed' then 1 end) as pass_count")
            ->selectRaw("count(case when results>>'$.pass_or_fail'= 'Failed' then 1 end) as failed_count")
            ...
            ->selectRaw("avg(results>>'$.percentage') as avg_percentage")
            ->selectRaw("avg(results>>'$.accuracy') as avg_accuracy")
            ->selectRaw("avg(results>>'$.speed') as avg_speed")
            ->selectRaw("max(results>>'$.score') as high_score")
            ->selectRaw("min(results>>'$.score') as low_score")
            ->selectRaw("sum(results>>'$.answered_questions') as sum_answered")
            ->selectRaw("sum(results>>'$.total_questions') as sum_questions")
            ->get();
    }
